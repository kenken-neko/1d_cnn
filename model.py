...
def build_1d_conv_model():
    ...
    model.add(
        keras.layers.Conv1D(
            filters=64, kernel_size=10, activation="relu", name="Conv1D_1"
        )
    )
    model.add(keras.layers.MaxPooling1D(pool_size=10, name="MaxPooling1D_1"))
    model.add(
        keras.layers.Conv1D(
            filters=64, kernel_size=5, activation="relu", name="Conv1D_2"
        )
    )
    model.add(keras.layers.MaxPooling1D(pool_size=5, name="MaxPooling1D_2"))
    model.add(
        keras.layers.Conv1D(
            filters=64, kernel_size=5, activation="relu", name="Conv1D_3"
        )
    )
